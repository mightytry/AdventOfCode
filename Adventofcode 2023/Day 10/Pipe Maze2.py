    ...
    for y in Tile.Map:
        ...
        for x in y:
            if (x.distance == -1 and is_inner):
                ...
                if (is_inner and len(n) != 0):
                    c += len(n)
                    for t in n: t.is_inner = True
                    n = []
                    is_inner = False
                elif (p != None and x in p.connections):
                    pass
                else:
                    is_inner = not is_inner
            p = x

    for x in range(len(Tile.Map[0])):
        is_inner = False
        n = []
        p = None
        for y in range(len(Tile.Map)):
            t = Tile.Map[y][x]
            if (t.distance == -1 and is_inner):
                n.append(t)
            elif (t.distance != -1):
                if (is_inner and len(n) != 0):
                    c += len(n)
                    ...
                    n = []
                    is_inner = False
                elif (p != None and t in p.connections):
                    pass
                else:
                    is_inner = not is_inner
            p = t
