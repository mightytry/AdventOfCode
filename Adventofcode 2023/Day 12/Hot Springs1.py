...
class Spring_Row:
    ...
    def __repr__(self) -> str:
        return str(self.springs) + "|" + str(self.contiguous_group)

@timer
def parse_data(data):
    for line in data.splitlines():
        #springs = [[Spring(sp) for sp in elm] for elm in line.split(" ")[0].replace("."," ").split()]
        springs = []
        previous_elm = ""
        for index,elm in enumerate(line.split(" ")[0]):
            if not(elm == "." and previous_elm == "."):
                springs.append(elm)
            previous_elm = elm

        contiguous_group = [int(elm) for elm in line.split(" ")[1].split(",")]
...
def main(data):
    ...
    return "\n".join(map(lambda x: repr(x.gen_solution()), data))
    return sum(map(lambda x: x.gen_solution(), data))
