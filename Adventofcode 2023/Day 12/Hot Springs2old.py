...
class Spring_Row:
    ...
    def eval(self):
        return self.count_matches(0,0,0)

    def count_matches(self, index, group, length):
        if group == self.contiguous_group_len and length != 0:
            return 0

        if group < self.contiguous_group_len and length > self.contiguous_group[group]:
            return 0
        ...
        if index == self.springs_len:
            if (a := (self.contiguous_group_len - group)) <= 1 and a * self.contiguous_group[self.contiguous_group_len-1] == length:
                return 1
            return 0
        ...
        if s in "#?":
            res = self.count_matches(index+1,group,length+1)
        if s in ".?":
            if length == 0:
                res += self.count_matches(index+1,group,0)
            elif length == self.contiguous_group[group]:
                res += self.count_matches(index+1,group+1,0)
